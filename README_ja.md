...
| レイヤー | 機能 | 得られるもの |
|---|---|---|
| 記憶 | 意味検索 — `POST /api/search`(ローカルベクトル、ユーザー分離、compact → full → pack) | 書き込み時に自動インデックス;自分の記憶だけを返す |
...
- ローカルファースト: データベース・ベクトル・ログはマシン内に留まる。匿名テレメトリは**デフォルトで無効**で、明示的に有効化した場合のみインストール ID・バージョン・OS・イベント名を送信し、記憶内容は送信しません。
