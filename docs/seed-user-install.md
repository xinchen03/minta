...
### 🅰 用 AI 编程工具的（Claude Code / Cursor / DSH）

```bash
# 1. 下载源码 ZIP（GitHub -> Code -> Download ZIP -> 解压）
# 2. 解压后，在文件夹里跑（Windows 双击 start_minta.bat 也行）
python minta_cli.py init     # 第一次：配环境（会问匿名统计，默认否）
python minta_cli.py start    # 起引擎（8772 控制台 + 18721 MCP）

# 3. 连你的编辑器
#    DSH:   dsh plugin --profile web add @xxinchen/dsh-plugin
#    Cursor: 见 docs/minta-mcp-config-guide.md
...
```bash
python minta_cli.py init && python minta_cli.py start
