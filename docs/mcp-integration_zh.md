...
```json
{
  "username": "字符串（必填）",
  "password": "字符串（必填）",
  "message": "字符串（必填）— 患者案例描述",
  "domain": "字符串（必填）— ankle_injury, knee_injury, cervical_spine_injury"
}
```

#### `minta_expert_list`

列出所有可用专家领域及其规则数量和信任度。
...
```json
{
  ...
  "password": "字符串（必填）",
  "message": "字符串（必填）— 案例描述",
  "primary_domain": "字符串（必填）",
  "consult_domain": "字符串（必填）"
}
```
...
```json
{
  ...
  "password": "字符串（必填）",
  "log_id": "整数（必填）",
  "signal": "字符串（必填）— positive 或 negative"
}
```
