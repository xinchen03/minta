...
4. v3 候选因此是一次**行为冻结快照**,不是配置变更:评测面代码与 `v2` 相同
   (评测面自 `v2` 起零改动),新增的是公开版产品侧修复与本文档；tag
   仍待提交前最后一步创建。
...
**解读**:重排是搜索延迟的支配项;其收益证据是
proxy 0.7422 vs 0.7329(n=861,±3pt CI 内)。若平台把搜索/总时间作为
资源约束或并列比较项,则"用 2.3–3.9× 搜索时间换 ≤1pt(CI 内)"是一个
需要显式确认的取舍。**本轮仍默认开启**，因为重排只改变证据顺序、不改写
内容；861 题全量点估计为正，未观察到类别级回归超出该区间；同时保留
`MINTA_EVAL_RERANK=0` 作为延迟受限场景的可复现回退。该决策与 README 的
透明披露一致。
