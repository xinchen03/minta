...
> 每个 AI 都将拥有记忆。问题不再是"它能记住吗？"——而是"它的记忆可信吗？"Minta 正在构建 AI 记忆的信任层。首先我们发现哪里出了问题。然后我们理解记忆之间的关系。然后我们预测会发生什么变化。
...
MIT 开源内核将永远覆盖个人记忆场景——单用户、单机、全质量治理。**Minta Pro** 将其扩展到团队和垂直领域：

| Pro 能力 | 意味着什么 | 状态 |
|----------|-----------|:----:|
| **多租户记忆** | 团队共享知识库，成员间隐私隔离。AI 知道团队知道什么——同时不该漏的绝不漏。 | 📋 规划中 |
| **领域专家模块** | 面向医学、法律、教育、工程的预标定规则包。不是通用提示词——是贴合每个领域推理模式的精调知识图谱。 | 🔬 研究中 |
| **可视化规则编辑器** | 拖拽式编辑器，让领域专家无需写代码即可定义记忆质量规则。律师、医生、教师——他们最懂自己领域里什么叫"正确"。我们把控制权交给他们。 | 📋 规划中 |
| **企业部署** | SSO、审计追踪、静态加密、本地部署。面向需要记忆治理但不能外传数据的团队。 | 📋 规划中 |
| **标定规则包** | 开源内核提供默认阈值。Pro 提供领域标定值——基于真实专家标注数据调优，而非启发式猜测。 | 🔬 研究中 |

> **分界线：** 记忆*机制*（检测、衰减、检索）是 MIT —— 永久开源。记忆*专业能力*（领域规则、标定阈值、垂直推理模式）是 Pro。你可以用机制构建任何东西。Pro 帮你省下标定时间。
