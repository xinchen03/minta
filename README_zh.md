...
> 每个 AI 都将拥有记忆。问题不再是"它能记住吗？"——而是"它的记忆可信吗？"Minta 正在构建 AI 记忆的信任层。首先发现哪里出了问题。然后理解记忆之间的关系。然后预测会发生什么变化。
...
MIT 开源内核覆盖个人记忆场景——单用户、单机、全质量治理。在此之上，我们正在推进：

**团队版**（规划中）
共享知识库 + 成员隐私边界。团队的 AI 知道团队知道什么——不该漏的绝不漏。面向工程团队、研究实验室，以及任何需要集体记忆长期保持正确的组织。

**领域专家模式**（研究中）

AI 不只是检索事实——它用领域判断力来推理事实。设计思路围绕几个核心理念：

- **联邦式，而非单体式。** 不是一个大模型试图无所不知，而是独立的领域专家各自深耕——医学、法律、教育、工程——并在边界处互相咨询。临床推理专家可以向法律推理专家寻求第二意见。系统会学习在什么问题上信任哪位专家，而每位专家也知道自己能力的边界。

- **记忆存在于图中，而非列表里。** 事实不是孤立的条目。它们被连接起来——依赖关系、矛盾关系、支撑证据——形成一个活的结构。当某条事实被推翻时，系统能回答"还有哪些记忆会因此改变？"

- **系统知道它知道什么——也知道它不知道什么。** 元知识贯穿每一层：每位专家追踪自己的判断记录，联邦将问题路由到正确的专业领域，超出能力范围时系统会拒绝作答。这就是"自信地给出错误答案"和"诚实地承认不知道"之间的区别。

- **置信度不是分数，是预测。** 每次专家判断背后，运行着一个持续学习记忆和观点如何随时间演化的模型。当专家输出与模型预测一致——信任累积。当现实偏离预测——某些东西变了，系统标出异常。随时间推移，它学会预判*应该*发生什么，并在世界偏离轨迹时察觉。一条与预测轨迹相悖的记忆不仅是意外——它是信号。

设计哲学很简单：领域专家——医生、律师、工程师——早就知道什么叫"正确"。他们只是不会写代码。我们把控制权交给他们。他们定义规则。AI 强制执行，而世界模型在背后捕捉规则漏掉的东西。

**长期愿景：个人专家 Agent**

每个知识工作者都应该有一个 AI，它从你的工作中学习，捕捉你看不到的盲点，随使用时间增长变得更有价值——而不是更臃肿。开源内核证明了机制可行。Pro 将这些扩展为产业级基础设施，让企业可以嵌入自己的产品，训练自己的领域专家，数据不出门。

> **分界线：** 记忆*机制*（检测、衰减、检索）是 MIT —— 永久开源。记忆*专业能力*（领域推理、标定判断、团队基础设施）是 Pro。你可以用机制构建任何东西。Pro 帮你省下数年的调优时间。

对 Pro 感兴趣？→ [xxinchen03@gmail.com](mailto:xxinchen03@gmail.com)
